...
namespace Electric.Application.AuditLog
{
    ...
    public class AuditLogHelper
    {
        ...
        /// <returns></returns>
        public static EleAuditLog GenerateRequestAuditLog(ActionContext actionContext, UserManager<EleUser> userManager)
        {
            //审核日志
            ...
            if (parameters.Length > 0)
            {
                ...
                foreach (var item in actionContext.ActionDescriptor.Parameters)
                {
                    var value = GetParameterValue(item, actionContext, parameters);
