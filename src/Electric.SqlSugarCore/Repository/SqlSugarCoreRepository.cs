...
namespace Electric.SqlSugarCore.Repository
{
        ...
        where TEntityPo : EntityPo<TKey>, new()
    {
        ...
        public async Task<TEntity> InsertAsync(TEntity entity, bool autoSave = false, CancellationToken cancellationToken = default)
        {
            ...
            var entityPo = _sqlSugarMapper.Map<TEntityPo>(entity);
            //是否有子表
            var haveIgnore = _db.EntityMaintenance.GetEntityInfo<TEntityPo>().Columns.Count(x => x.IsIgnore) > 0;
            ...
            {
                var command = _db.Insertable(entityPo);
                if (autoSave)
                {
                    await command.ExecuteCommandAsync();
                }
        ...
        public async Task InsertManyAsync(IEnumerable<TEntity> entities, bool autoSave = false, CancellationToken cancellationToken = default)
        {
            ...
            {
                var command = _db.Insertable(entityPos);
                if (autoSave)
                {
                    await command.ExecuteCommandAsync();
                }
        ...
        public async Task<TEntity> UpdateAsync(TEntity entity, bool autoSave = false, CancellationToken cancellationToken = default)
        {
            ...
            var entityPo = _sqlSugarMapper.Map<TEntityPo>(entity);
            //是否有子表
            var haveIgnore = _db.EntityMaintenance.GetEntityInfo<TEntityPo>().Columns.Count(x => x.IsIgnore || x.IsOnlyIgnoreUpdate) > 0;

            if(haveIgnore)
            {
                //获取忽略更新的列
                var ignoreColumns = _db.EntityMaintenance.GetEntityInfo<TEntityPo>().Columns.Where(x => x.IsOnlyIgnoreUpdate).Select(x=>x.DbColumnName).ToArray();
                var command = _db.UpdateNav(entityPo).IncludesAllFirstLayer(ignoreColumns);
            ...
            {
                var command = _db.Updateable(entityPo);
                if (autoSave)
                {
                    await command.ExecuteCommandAsync();
                }
        ...
        public async Task UpdateManyAsync(IEnumerable<TEntity> entities, bool autoSave = false, CancellationToken cancellationToken = default)
        {
            ...
            if(haveIgnore)
            {
                //获取忽略更新的列
                var ignoreColumns = _db.EntityMaintenance.GetEntityInfo<TEntityPo>().Columns.Where(x => x.IsOnlyIgnoreUpdate).Select(x => x.DbColumnName).ToArray();
                var command = _db.UpdateNav(entityPos).IncludesAllFirstLayer(ignoreColumns);
                if (autoSave)
                {
                    await command.ExecuteCommandAsync();
                }
            ...
            {
                var command = _db.Updateable(entityPos);
                if (autoSave)
                {
                    await command.ExecuteCommandAsync();
                }
